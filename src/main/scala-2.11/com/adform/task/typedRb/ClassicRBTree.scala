...
abstract sealed class Tree[A](implicit ev1: A ⇒ Ordered[A]) {
  ...
  def add(elem: A)(implicit ev1: A ⇒ Ordered[A]): Tree[A] = {
    ...
    def balancedAdd(t: Tree[A]): Tree[A] = //balancedAdd - just adds element to the left or right (like in any binary tree)
    ...
    def rotate(z: A, y: A, x: A, a: T, b: T, c: T, d: T): T = {
    ...
    def balanceRight(c: Color, x: A, l: T, r: T) = (c, l, r) match {
...
case class T[A <% Ordered[A]](color: Color,
                              value: A,
                              left: Tree[A] = Leaf.asInstanceOf[Tree[A]],
                              right: Tree[A] = Leaf.asInstanceOf[Tree[A]]) extends Tree[A] {
  def isEmpty = false
}
...
object Tree {
  ...
  def apply[A <% Ordered[A]](xs: A*): Tree[A] = {
    var r: Tree[A] = Leaf.asInstanceOf[Tree[A]]
    for (x <- xs) r = r.add(x)
    r
  }
}